...
namespace InduMovel.Areas.Admin.Services
{
    public class RelatorioVendasServices
    {
        ...
        public async Task<List<Pedido>> BuscaPorData(DateTime? di, DateTime? df)
        {
            ...
            var queryPedidos = from obj in _context.Pedidos select obj; if (di.HasValue)
            {
            ...
            return await queryPedidos.Include(i => i.PedidoMoveis).ThenInclude(m => m.Movel).OrderBy(d => d.PedidoEnviado).ToListAsync();
        }
    }
}
